...
type ActiveTab =
  "terminal" | "logs" | "settings" | "files" | "system" | "sqlite";
...
const MAIN_SIDEBAR_WIDTH_KEY = "vps_terminal_sidebar_width";
const SQLITE_SIDEBAR_WIDTH_KEY = "vps_terminal_sqlite_sidebar_width";
...
function previewKind(filePath: string): PreviewKind {
  ...
  if (["mp3", "wav", "ogg", "oga", "aac", "m4a", "flac", "opus"].includes(ext))
    return "audio";
  if (
    ["png", "jpg", "jpeg", "gif", "webp", "avif", "bmp", "svg", "ico"].includes(
      ext,
    )
  )
    return "image";
...
function SqliteResultTable({
  ...
  if (!rows.length)
    return (
      <div className="p-8 text-center text-xs text-slate-600">{empty}</div>
    );
  return (
...
export default function Home() {
  ...
  const displayedSocketStatus = activeTab === "terminal" ? socketStatus : "idle";
  ...
  return (
    <div className="app-shell app-grid-bg flex flex-col min-h-screen text-slate-300 font-sans selection:bg-blue-500/30 selection:text-white">
              ...
              <div className="hidden lg:flex flex-col justify-between p-10 border-r border-white/10 bg-[radial-gradient(circle_at_20%_20%,rgba(56,189,248,0.12),transparent_45%)]">
                <div>
                  ...
                  <p className="mt-4 max-w-md text-sm leading-6 text-slate-400">
                    Terminal thời gian thực, quản lý tệp, dịch vụ hệ thống,
                    SQLite Studio và nhật ký kiểm toán trong cùng một phiên bảo
                    mật.
                  </p>
                </div>
                <div className="grid grid-cols-3 gap-3">
                  <div className="rounded-xl border border-white/10 bg-black/20 p-4">
                    ...
                    <span className="mt-3 block text-[10px] uppercase tracking-wider text-slate-500">
                      Live shell
                    </span>
                  </div>
                  <div className="rounded-xl border border-white/10 bg-black/20 p-4">
              ...
              <div className="relative p-6 sm:p-10">
                ...
                <div className="text-center mb-8">
                  ...
                  <p className="text-sm text-slate-500 mt-1">
                    {twoFactorChallenge
                      ? "Nhập mã 6 số hoặc mã khôi phục"
                      : "Nhập khóa truy cập VPS để khởi tạo Node-PTY"}
                  </p>
                </div>

                <form
                  onSubmit={
                    twoFactorChallenge ? handleTwoFactorLogin : handleLogin
                  }
                  className="space-y-5"
                >
                  {!twoFactorChallenge && (
                    <input
                      ...
                      className="app-input w-full py-3 px-4 text-center text-white focus:outline-none"
                    />
                  )}
                  <div>
                    <input
                  ...
                  >
                    {loading ? (
                      ...
                      <>
                        ...
                        <span>
                          {twoFactorChallenge ? "XÁC NHẬN MÃ" : "KẾT NỐI SHELL"}
                        </span>
                      </>
                    )}
                  </button>
            ...
            <header className="app-topbar h-14 sm:h-16 border-b flex items-center justify-between px-3 sm:px-6 shrink-0 z-50">
              <div className="flex items-center gap-4">
                ...
                <h1 className="text-xs sm:text-sm font-bold tracking-tight text-white flex items-center gap-2">
                  ...
                  <span className="rounded-full border border-white/10 bg-white/5 px-2 py-0.5 text-[8px] text-slate-500 font-mono font-normal">
                    v1.1
                  </span>
                </h1>
              </div>
              ...
              <div className="flex items-center gap-4 sm:gap-8">
                <div className="hidden md:flex items-center gap-6">
                  <div className="text-right">
                    <p className="text-[9px] uppercase text-slate-500 leading-none mb-1">
                      Cơ sở dữ liệu
                    </p>
                    <p className="text-xs font-mono text-blue-400 font-semibold uppercase">
                      SQLite Đang Hoạt Động
                    </p>
                  </div>
                  ...
                  <div className="text-right">
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full inline-block ${displayedSocketStatus === "connected" ? "bg-emerald-400" : displayedSocketStatus === "connecting" ? "bg-amber-400 animate-pulse" : displayedSocketStatus === "error" ? "bg-rose-400" : "bg-slate-500"}`}></span>
                      {displayedSocketStatus === "connected" ? "ĐÃ KẾT NỐI" : displayedSocketStatus === "connecting" ? "ĐANG KẾT NỐI" : displayedSocketStatus === "error" ? "LỖI KẾT NỐI" : "SẴN SÀNG"}
                    </p>
                  </div>
                </div>
                ...
                >
                  <span className="hidden sm:inline">Chấm dứt phiên</span>
            ...
            <div className="flex flex-1 overflow-hidden">
              {isSidebarOpen && (
                <button
                  ...
                  className="fixed inset-x-0 top-14 bottom-8 z-30 bg-black/60 backdrop-blur-sm md:hidden"
                />
              )}
              {/* COLLAPSIBLE SIDEBAR */}
              <AnimatePresence initial={false}>
                {isSidebarOpen && (
                  <motion.aside
                    ...
                    animate={{ width: mainSidebarWidth, opacity: 1 }}
                    exit={{ width: 0, opacity: 0 }}
                    className="app-sidebar relative border-r flex flex-col h-full shrink-0 overflow-y-auto"
                  >
                    {/* System Resource Widgets */}
                    <div className="p-4 border-b border-white/10 space-y-3">
                      <div className="flex items-center justify-between">
                        <h3 className="app-kicker">Tài nguyên</h3>
                        ...
                        {currentUser &&
                          ["admin", "root"].includes(currentUser.role) && (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-xs font-semibold uppercase tracking-wider transition border ${activeTab === "system" ? "bg-sky-500/10 text-sky-300 border-sky-400/20 shadow-[inset_3px_0_0_#38bdf8]" : "text-slate-400 border-transparent hover:bg-white/5 hover:text-white"}`}
                            >
                        ...
                        {currentUser &&
                          ["admin", "root"].includes(currentUser.role) && (
                            <button
                              ...
                              className={`w-full flex items-center gap-3 px-3 py-2.5 rounded text-xs font-semibold uppercase tracking-wider transition border ${activeTab === "sqlite" ? "bg-emerald-500/10 text-emerald-300 border-emerald-400/20 shadow-[inset_3px_0_0_#34d399]" : "text-slate-400 border-transparent hover:bg-white/5 hover:text-white"}`}
                            >
                              <Database className="w-4 h-4" />
                              <span>Quản Lý SQLite</span>
                            </button>
                          )}
                      ...
                      <div className="space-y-3 font-mono text-[11px] leading-relaxed">
                        ...
                        {logs.length === 0 && (
                          <p className="text-slate-600 italic">
                            Chưa ghi nhận lượt kết nối nào.
                          </p>
                        )}
                      </div>
                    </div>
                    <div
                      ...
                      onPointerDown={(event) => startResize(event, "main")}
                      className="sidebar-resizer right-0"
                    />
                  </motion.aside>
                )}
              </AnimatePresence>
                  ...
                  <AnimatePresence mode="wait">
                          ...
                          <div className="max-w-6xl mx-auto space-y-6">
                            <div className="workspace-heading flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                              <div>
                                ...
                                <p className="text-xs text-slate-500 font-mono">
                                  {logTotal} sự kiện xác thực, terminal và
                                  filesystem
                                </p>
                              </div>
                              <div className="flex flex-wrap gap-2">
                                <button
                                  onClick={checkLogIntegrity}
                                  className={`px-3 py-1.5 text-xs border rounded ${logIntegrity?.valid === false ? "text-red-400 border-red-500/30" : logIntegrity?.valid ? "text-emerald-400 border-emerald-500/30" : "border-white/10"}`}
                                >
                                  {logIntegrity
                                    ? logIntegrity.valid
                                      ? `Chuỗi hợp lệ (${logIntegrity.checked})`
                                      : `Chuỗi lỗi tại #${logIntegrity.brokenAt}`
                                    : "Kiểm tra toàn vẹn"}
                                </button>
                                <button
                            ...
                            >
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  type="submit"
                                  className="px-3 bg-blue-600 rounded text-xs"
                                >
                                  Lọc
                                </button>
                              </div>
                            ...
                            <div className="flex items-center justify-between text-xs text-slate-500">
                              <span>
                                {logTotal
                                  ? `${logOffset + 1}-${Math.min(logOffset + logs.length, logTotal)} / ${logTotal}`
                                  : "0 kết quả"}
                              </span>
                              <div className="flex gap-2">
                                <button
                                  ...
                                  onClick={() =>
                                    loadLogs(Math.max(0, logOffset - 50))
                                  }
                                ...
                                <button
                                  disabled={logOffset + logs.length >= logTotal}
                          ...
                          <div className="max-w-7xl mx-auto space-y-5">
                            <div className="flex flex-col sm:flex-row sm:items-center gap-3 border-b border-white/10 pb-4">
                              ...
                              <div className="flex rounded border border-white/10 overflow-hidden">
                                <button
                                  onClick={() => setSystemView("services")}
                                  className={`px-3 py-2 text-xs ${systemView === "services" ? "bg-blue-600 text-white" : "bg-black"}`}
                                >
                                  Services
                                </button>
                                <button
                                  onClick={() => setSystemView("processes")}
                                  className={`px-3 py-2 text-xs ${systemView === "processes" ? "bg-blue-600 text-white" : "bg-black"}`}
                                >
                                  Processes
                                </button>
                              </div>
                              ...
                              >
                                <RefreshCw
                                  className={`inline w-3.5 h-3.5 mr-1 ${systemLoading ? "animate-spin" : ""}`}
                                />
                                Tải lại
                              </button>
                            </div>
                            ...
                            <div className="workspace-heading flex flex-col sm:flex-row sm:items-center gap-3">
                              <div className="mr-auto">
                                ...
                                <p className="mt-1 text-xs text-slate-500 font-mono">
                                  Browse, mutate, inspect and safeguard server
                                  databases
                                </p>
                              </div>
                              <button
                                ...
                                {!selectedSqlite ? (
                                  <div className="min-h-96 rounded-xl border border-dashed border-white/10 bg-[radial-gradient(circle_at_center,rgba(16,185,129,0.05),transparent_60%)] flex flex-col items-center justify-center text-center p-8">
                                    ...
                                    <p className="mt-1 max-w-sm text-xs text-slate-500">
                                      Khám phá bảng, chỉnh sửa dữ liệu, chạy
                                      truy vấn và quản lý backup từ một
                                      workspace.
                                    </p>
                                  </div>
                                ) : (
                                    ...
                                    <div className="rounded-xl border border-white/10 bg-[#0d1110] overflow-hidden">
                                      <div className="p-4 flex flex-col sm:flex-row gap-3 sm:items-center">
                                        ...
                                        {currentUser.role === "root" &&
                                          !sqliteFiles.find(
                                            (file) =>
                                              file.path === selectedSqlite,
                                          )?.protected && (
                                            <button
                                              onClick={() =>
                                                deleteSqlite(selectedSqlite)
                                              }
                                        ...
                                        <div className="min-w-0 space-y-4">
                                          <div className="rounded-xl border border-white/10 bg-[#0d1110] overflow-hidden">
                                            ...
                                            <textarea
                                              value={sqliteSql}
                                              onChange={(event) =>
                                                setSqliteSql(event.target.value)
                                              }
                                              spellCheck={false}
                                        ...
                                        <aside className="rounded-xl border border-white/10 bg-[#0d1110] overflow-hidden self-start">
                                          ...
                                          <div className="max-h-[60vh] overflow-y-auto divide-y divide-white/5">
                                            {sqliteHistory
                                              .filter(
                                                (entry) =>
                                                  entry.path === selectedSqlite,
                                              )
                                              .map((entry) => (
                                                <button
                                                  key={entry.id}
                                                  onClick={() =>
                                                    setSqliteSql(entry.sql)
                                                  }
                                            ...
                                            {!sqliteHistory.some(
                                              ...
                                              <p className="p-6 text-center text-[10px] text-slate-600">
                                                Chưa có truy vấn trong database
                                                này.
                                              </p>
                                            )}
                                          </div>
                                        ...
                                        <div className="rounded-xl border border-emerald-500/15 bg-[#0d1110] p-4 space-y-4 self-start">
                                          <div>
                                            ...
                                            <p className="mt-1 text-[10px] text-slate-600">
                                              DDL actions are sent as structured
                                              payloads.
                                            </p>
                                          </div>
                                          <select
                                            ...
                                            onChange={(event) =>
                                              setSqliteSchemaAction(
                                                event.target
                                                  .value as typeof sqliteSchemaAction,
                                              )
                                            }
                                            className="w-full bg-black border border-white/10 rounded px-3 py-2 text-xs"
                                          >
                                            <option value="createTable">
                                              Create table
                                            </option>
                                            <option value="addColumn">
                                              Add column
                                            </option>
                                            <option value="createIndex">
                                              Create index
                                            </option>
                                            <option value="dropIndex">
                                              Drop index
                                            </option>
                                            <option value="dropTable">
                                              Drop table
                                            </option>
                                          </select>
                                          {sqliteSchemaAction ===
                        ...
                        <div className="max-w-3xl mx-auto space-y-8">
                          ...
                          <div className="app-panel p-4 sm:p-6 space-y-6">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/5 pb-4">
                              <div>
                                ...
                                <p className="text-xs text-slate-500 font-mono">
                                  Chỉnh sửa các tùy chọn thiết lập được lưu vào
                                  cấu hình cục bộ
                                </p>
                              </div>
                              <AnimatePresence>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              ...
                              <div>
                                ...
                                >
                                  ...
                                  <option value="matrix">
                                    Xanh lục Ma trận (Cổ điển)
                                  </option>
                                  <option value="amber">
                                    Cam Hổ phách (CRT Phosphor)
                                  </option>
                                  <option value="cyberpunk">
                                    Neon Cyberpunk (Xanh & Hồng)
                                  </option>
                                  <option value="dracula">
                                    Dracula (Tím cổ điển)
                                  </option>
                                  <option value="tokyo-night">
                                    Tokyo Night (Xanh đêm)
                                  </option>
                                  <option value="nord">
                                    Nord (Băng giá dịu mắt)
                                  </option>
                            ...
                            <div className="border-t border-white/5 pt-6 space-y-4">
                              ...
                              <div
                                style={{
                                  backgroundColor:
                                    getTerminalColors(activePreviewTheme)
                                      .background,
                                  color:
                                    getTerminalColors(activePreviewTheme)
                                      .foreground,
                          ...
                          <div className="p-6 rounded-xl border border-white/10 bg-[#111116]/40 space-y-6">
                            <div>
                              ...
                              <p className="text-xs text-slate-500 font-mono">
                                Cập nhật mật khẩu chính để bảo vệ quyền truy cập
                                Node-PTY
                              </p>
                            </div>
                          ...
                          <div className="p-6 rounded-xl border border-white/10 bg-[#111116]/40 space-y-6">
                            <div className="flex items-start justify-between gap-4">
                              <div>
                                ...
                                <p className="text-xs text-slate-500 font-mono">
                                  TOTP tương thích Google Authenticator, Authy
                                  và 1Password
                                </p>
                              </div>
                              <span
                            ...
                            {!securityStatus?.twoFactorEnabled ? (
                                  ...
                                  <div className="grid md:grid-cols-[180px_1fr] gap-5 items-center">
                                    ...
                                    <div className="space-y-3 min-w-0">
                                      <p className="text-xs text-slate-400">
                                        Quét QR rồi nhập mã 6 số để xác nhận. Có
                                        thể nhập secret thủ công:
                                      </p>
                              ...
                              <div className="space-y-3">
                                <p className="text-xs text-slate-400">
                                  Còn {securityStatus.recoveryCodesRemaining} mã
                                  khôi phục. Để tắt 2FA, nhập mật khẩu và mã
                                  TOTP hoặc recovery code.
                                </p>
                            ...
                            {recoveryCodes.length > 0 && (
                              <div className="p-4 rounded border border-amber-500/30 bg-amber-500/5 space-y-3">
                                <p className="text-xs font-semibold text-amber-300">
                                  Các mã này chỉ hiển thị một lần. Lưu ở nơi an
                                  toàn.
                                </p>
                          ...
                          {currentUser?.role === "root" && (
                            <div className="p-6 rounded-xl border border-white/10 bg-[#111116]/40 space-y-5">
                              <div>
                                ...
                                <p className="text-xs text-slate-500 font-mono">
                                  Viewer chỉ xem; Operator sửa file; Admin có
                                  terminal; Root toàn quyền.
                                </p>
                              </div>
                              ...
                              >
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <p className="text-[11px] text-slate-500 font-mono mt-0.5">
                                    Cấu hình giao diện đã đồng bộ thành công vào
                                    máy chủ.
                                  </p>
                                </div>
                                <button
                        ...
                        <div className="max-w-6xl mx-auto space-y-6">
                          ...
                          <div className="workspace-heading flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                            <div>
                              ...
                              <p className="text-xs text-slate-500 font-mono">
                                Duyệt, xem, tạo, sửa và xóa tệp tin trên hệ
                                thống VPS
                              </p>
                            ...
                            <div className="flex items-center gap-2 overflow-x-auto pb-1">
                              ...
                              {fileBookmarks.map((bookmark) => (
                                ...
                                >
                                  ...
                                  <button
                                    onClick={() =>
                                      toggleFileBookmark(bookmark.path)
                                    }
                                ...
                                <div className="p-4 bg-black">
                                  <div className="flex flex-wrap gap-2 mb-2">
                                    ...
                                    <input
                                      value={editorReplace}
                                      onChange={(e) =>
                                        setEditorReplace(e.target.value)
                                      }
                                  ...
                                  <div className="mt-2 text-right text-[10px] font-mono text-slate-500">
                                    Dòng {editorPosition.line}, ký tự{" "}
                                    {editorPosition.char} |{" "}
                                    {(fileContent || "").length} ký tự | Ctrl+S
                                    để lưu
                                  </div>
                                </div>
                              )}
                              ...
                              <div className="flex flex-wrap gap-2 p-2 border-b border-white/10 text-[11px]">
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    if (!name) return;
                                    const format = name.endsWith(".tar.gz")
                                      ? "tar.gz"
                                      : name.split(".").pop() || "zip";
                                    try {
                                      await requestFileApi(
                                ...
                                <button
                                  onClick={async () => {
                                    ...
                                    const targetPath =
                                      name && prompt("Đường dẫn đích:")?.trim();
                                                ...
                                                <button
                                                  onClick={async () => {
                                                    try {
                                                      const data =
                                                        await requestFileApi(
                                                          `/api/files/metadata?path=${encodeURIComponent(fullItemPath)}`,
                                                        );
                                                      setMetadata({
                                                        path: fullItemPath,
                                                        mode: String(
                    ...
                    {metadata && (
                      <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="app-modal w-full max-w-md p-5 space-y-4">
                          <div className="flex">
                          ...
                          <label className="block text-xs">
                            ...
                            <input
                              ...
                              className="mt-1 w-full bg-black border border-white/10 rounded p-2 font-mono"
                            />
                          </label>
                          <div className="grid grid-cols-2 gap-3">
                            <label className="text-xs">
                              ...
                              <input
                                ...
                                className="mt-1 w-full bg-black border border-white/10 rounded p-2"
                              />
                            </label>
                            <label className="text-xs">
                              GID
                          ...
                          <button
                            ...
                            className="w-full bg-blue-600 rounded py-2 text-sm text-white"
                          >
                            Lưu quyền
                          </button>
                        </div>
                    ...
                    {showTrash && (
                      <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="app-modal w-full max-w-2xl max-h-[80vh] overflow-auto p-5 space-y-4">
                          <div className="flex items-center">
                            <h4 className="font-bold text-white">Thùng rác</h4>
                            <button
                          ...
                          {trashItems.length === 0 ? (
                            ...
                            trashItems.map((item) => (
                              <label
                                key={item.id}
                                className="flex gap-3 p-3 border border-white/5 rounded text-xs"
                              >
                                <input
                                  type="checkbox"
                          ...
                          <div className="flex flex-wrap gap-2">
                            <button
                              ...
                              onClick={() =>
                                trashAction("restore", selectedTrashIds)
                              }
                              className="px-3 py-2 bg-emerald-600 rounded text-xs disabled:opacity-30"
                            >
                              Khôi phục
                            </button>
                            <button
                              ...
                              className="px-3 py-2 bg-red-600 rounded text-xs disabled:opacity-30"
                            >
                              Xóa vĩnh viễn
                            </button>
                            <button
                              ...
                              className="ml-auto px-3 py-2 bg-red-950 text-red-300 rounded text-xs"
                            >
                              Dọn sạch
                            </button>
                          </div>
                    ...
                    {showSnapshots && (
                      <div className="fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-center justify-center p-4">
                        <div className="app-modal w-full max-w-4xl max-h-[85vh] overflow-auto p-5 space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] text-slate-500 font-mono">
                                {snapshotPath || "Tất cả tệp"} ·{" "}
                                {snapshots.length} phiên bản
                              </p>
                            </div>
                            <button
                          ...
                          {snapshots.length === 0 ? (
                            ...
                            <div className="space-y-2">
                              {snapshots.map((snapshot) => (
                                <div
                                  key={snapshot.id}
                                  className="grid md:grid-cols-[1fr_180px_auto] gap-3 items-center p-3 rounded border border-white/5 bg-black/40"
                                >
                                  <div className="min-w-0">
                                    ...
                                    <code
                                      className="block mt-1 text-[9px] text-slate-600 truncate"
                    ...
                    {serviceLogs && (
                      <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4">
                        <div className="w-full max-w-5xl max-h-[85vh] flex flex-col rounded-xl border border-white/10 bg-[#111116] p-5 gap-4">
                          <div className="flex items-center">
                            <div>
                              <h4 className="font-bold text-white">
                                Journal: {serviceLogs.unit}
                              </h4>
                            ...
                            <button
                              onClick={() => setServiceLogs(null)}
                              className="ml-auto"
                            >
                              <X className="w-4 h-4" />
                            </button>
                          </div>
                          <pre className="flex-1 overflow-auto whitespace-pre-wrap break-words bg-black border border-white/5 rounded p-4 text-[11px] leading-relaxed text-slate-300 font-mono">
                            {serviceLogs.logs || "Không có log."}
                          </pre>
                        </div>
                      </div>
                    )}
                    {sqliteRecordModal && (
                      <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-4">
                        <div className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-xl border border-emerald-500/20 bg-[#0d1110] shadow-2xl">
                          <div className="sticky top-0 z-[1] flex items-center gap-3 p-5 border-b border-white/10 bg-[#0d1110]">
                            <Table2 className="w-5 h-5 text-emerald-400" />
                            <div>
                              <h4 className="font-bold text-white">
                                {sqliteRecordModal.mode === "add"
                                  ? "Thêm bản ghi"
                                  : "Chỉnh sửa bản ghi"}
                              </h4>
                              <p className="mt-1 text-[10px] text-slate-500 font-mono">
                                {selectedSqliteTable} · nhập chính xác{" "}
                                <span className="text-emerald-400">NULL</span>{" "}
                                để lưu giá trị null
                              </p>
                            </div>
                            <button
                              onClick={() => setSqliteRecordModal(null)}
                              className="ml-auto"
                            >
                              <X className="w-4 h-4" />
                            </button>
                          ...
                          <div className="grid sm:grid-cols-2 gap-3 p-5">
                            {Object.entries(sqliteRecordValues).map(
                              ([column, value]) => (
                                <label
                                  key={column}
                                ...
                                >
                                  <span className="flex items-center gap-2">
                                    ...
                                    {sqliteColumns.find(
                                      ...
                                      <span className="rounded bg-amber-500/10 px-1.5 py-0.5 text-[8px] text-amber-400">
                                        PK
                                      </span>
                                    )}
                                  </span>
                                  <textarea
                                    ...
                                    onChange={(event) =>
                                      setSqliteRecordValues((values) => ({
                                        ...values,
                                        [column]: event.target.value,
                                      }))
                                    }
                                    rows={2}
                                    className="mt-1.5 w-full resize-y rounded border border-white/10 bg-black p-2.5 text-xs font-mono normal-case tracking-normal text-white outline-none focus:border-emerald-500/40"
                                  />
                                </label>
                              ),
                            )}
                          </div>
                          <div className="sticky bottom-0 flex justify-end gap-2 p-4 border-t border-white/10 bg-[#0d1110]">
                            <button
                              onClick={() => setSqliteRecordModal(null)}
                            ...
                            <button
                              onClick={saveSqliteRecord}
                              disabled={sqliteLoading}
                              className="flex items-center gap-2 px-4 py-2 text-xs bg-emerald-500 text-black font-bold rounded disabled:opacity-40"
                            >
                              <Save className="w-3.5 h-3.5" />
                              {sqliteRecordModal.mode === "add"
                                ? "Thêm bản ghi"
                    ...
                    {showSqliteBrowser && (
                      <div className="fixed inset-0 z-50 bg-black/85 flex items-center justify-center p-3 sm:p-4">
                        <div className="w-full max-w-4xl max-h-[88vh] flex flex-col rounded-xl border border-emerald-500/20 bg-[#0d1110] shadow-2xl overflow-hidden">
                          <div className="flex items-start gap-3 p-5 border-b border-white/10">
                            ...
                            <div>
                              ...
                              <p className="mt-1 text-[10px] text-slate-500 font-mono">
                                Duyệt filesystem từ{" "}
                                {sqliteBrowserRoot || "root"} · chỉ hiện file
                                SQLite hợp lệ
                              </p>
                            </div>
                            <button
                              onClick={() => setShowSqliteBrowser(false)}
                              className="ml-auto"
                            >
                              <X className="w-4 h-4" />
                            </button>
                          ...
                          <form
                            onSubmit={(event) => {
                              event.preventDefault();
                              browseSqlitePath(sqliteBrowserPath);
                            }}
                            className="flex gap-2 p-3 border-b border-white/10"
                          >
                            <button
                              type="button"
                              disabled={!sqliteBrowserParent}
                              onClick={() =>
                                sqliteBrowserParent &&
                                browseSqlitePath(sqliteBrowserParent)
                              }
                              title="Thư mục cha"
                              className="p-2.5 rounded border border-white/10 disabled:opacity-30"
                            >
                              <ArrowLeft className="w-4 h-4" />
                            </button>
                            <input
                              value={sqliteBrowserPath}
                              onChange={(event) =>
                                setSqliteBrowserPath(event.target.value)
                              }
                              placeholder="/var/lib/app"
                              className="min-w-0 flex-1 bg-black border border-white/10 focus:border-emerald-500/50 outline-none rounded px-3 py-2 text-xs text-white font-mono"
                            />
                            <button
                              type="submit"
                              className="px-4 py-2 rounded bg-white/5 border border-white/10 text-xs"
                            >
                              Đi tới
                            </button>
                          </form>
                          <div className="flex-1 min-h-64 overflow-y-auto divide-y divide-white/5">
                            {sqliteBrowserItems.map((item) => (
                              <button
                                key={item.path}
                                onClick={() =>
                                  item.type === "directory"
                                    ? browseSqlitePath(item.path)
                                    : selectExistingSqlite(item.path)
                                }
                                className="w-full flex items-center gap-3 p-3 sm:px-5 text-left hover:bg-white/[0.03]"
                              >
                                <span
                                  className={`p-2 rounded ${item.type === "directory" ? "bg-blue-500/10 text-blue-400" : "bg-emerald-500/10 text-emerald-400"}`}
                                >
                                  {item.type === "directory" ? (
                                    <Folder className="w-4 h-4" />
                                  ) : (
                                ...
                                <span className="min-w-0 flex-1">
                                  ...
                                  <span className="block mt-1 text-[9px] text-slate-600 font-mono truncate">
                                    {item.path}
                                  </span>
                                </span>
                                {item.type === "database" ? (
                            ...
                            {sqliteBrowserItems.length === 0 && (
                              <div className="p-12 text-center text-xs text-slate-600">
                                Thư mục trống hoặc không có file SQLite hợp lệ.
                              </div>
                            )}
                          </div>
                          <div className="flex items-center justify-between gap-3 p-3 border-t border-white/10 text-[9px] text-slate-600 font-mono">
                            <span className="truncate">
                              Root: {sqliteBrowserRoot}
                            </span>
                            <button
                              onClick={() => setShowSqliteBrowser(false)}
                              className="px-3 py-1.5 text-xs border border-white/10 rounded text-slate-300"
                            >
                              Đóng
                            </button>
                          </div>
            ...
            <footer className="status-footer h-8 border-t flex items-center justify-between px-3 sm:px-6 shrink-0 text-[9px] sm:text-[10px] font-mono text-slate-500 uppercase tracking-widest z-50 select-none">
              <div className="flex gap-6">
                <span className="flex items-center gap-1.5">
                  <span className={`w-1.5 h-1.5 rounded-full ${displayedSocketStatus === "connected" ? "bg-emerald-400" : displayedSocketStatus === "connecting" ? "bg-amber-400 animate-pulse" : displayedSocketStatus === "error" ? "bg-rose-400" : "bg-slate-500"}`}></span>
                  Socket.io: {displayedSocketStatus === "connected" ? "Kết nối" : displayedSocketStatus === "connecting" ? "Đang nối" : displayedSocketStatus === "error" ? "Lỗi" : "Sẵn sàng"}
                </span>
      ...
      {paletteOpen && isAuthenticated && (
        <div
          className="fixed inset-0 z-[120] bg-black/70 backdrop-blur-sm flex items-start justify-center p-3 pt-[12vh]"
          role="dialog"
          aria-modal="true"
          ...
          <div className="command-palette app-modal w-full max-w-xl overflow-hidden">
            <div className="flex items-center gap-3 border-b border-white/10 px-4">
              ...
              <input
                ...
                onKeyDown={(event) => {
                  if (event.key === "Enter" && paletteActions[0])
                    paletteActions[0].run();
                }}
                placeholder="Tìm workspace hoặc thao tác..."
                className="h-14 min-w-0 flex-1 bg-transparent text-sm text-white outline-none"
                aria-label="Tìm lệnh"
              />
              <kbd>Esc</kbd>
            </div>
            ...
            <div className="border-t border-white/10 px-4 py-2 text-[10px] text-slate-600 font-mono">
              Enter để mở · Alt+1..6 chuyển workspace
            </div>
          </div>
        </div>
      )}
        ...
        >
          <form
            onSubmit={(event) => {
              event.preventDefault();
              if (
                !confirmPrompt.requiredText ||
                confirmText === confirmPrompt.requiredText
              )
                closeConfirm(true);
            }}
            className={`app-modal w-full max-w-md p-6 space-y-5 ${confirmPrompt.danger ? "!border-rose-500/30" : ""}`}
          >
            <div className="flex items-start gap-3">
              <AlertCircle
                className={`w-5 h-5 shrink-0 ${confirmPrompt.danger ? "text-rose-400" : "text-sky-400"}`}
              />
              <div>
                <h3 id="confirm-title" className="font-bold text-white">
                  {confirmPrompt.title || "Xác nhận thao tác"}
                </h3>
                <p
                  id="confirm-message"
            ...
            {confirmPrompt.requiredText && (
              <label className="block text-xs text-slate-400">
                Nhập{" "}
                <code className="text-rose-300 break-all">
                  {confirmPrompt.requiredText}
                </code>{" "}
                để tiếp tục
                <input
                  ...
                  className="mt-2 w-full bg-black border border-white/10 rounded px-3 py-2 text-sm text-white font-mono"
                  autoComplete="off"
                />
              </label>
            )}
            <div className="flex justify-end gap-2">
              <button
                type="button"
                autoFocus={!confirmPrompt.requiredText}
              ...
              <button
                ...
                className={`px-4 py-2 text-xs font-bold rounded disabled:opacity-30 ${confirmPrompt.danger ? "bg-rose-600 text-white" : "bg-sky-500 text-black"}`}
              >
                {confirmPrompt.confirmLabel || "Xác nhận"}
              </button>
            </div>
          </form>
        </div>
      ...
      {contextMenu && (
        ...
        >
          <p className="context-menu-title">{contextMenu.item.name}</p>
            ...
            <>
              <button
                ...
                onClick={() => {
                  ...
                  else openFile(contextMenu.item.path);
                  setContextMenu(null);
                }}
              >
                {contextMenu.item.isDirectory ? "Mở thư mục" : "Xem tệp"}
              </button>
              ...
              {currentUser?.role !== "viewer" && (
                <button
                  role="menuitem"
                  onClick={() => {
                    moveOrRename(contextMenu.item.path);
                    setContextMenu(null);
                  }}
                >
                  Đổi tên
              ...
              {currentUser?.role !== "viewer" && (
                <button
                  ...
                  className="danger"
                  onClick={() => {
                    deleteFileOrFolder(contextMenu.item.path);
                    setContextMenu(null);
                  }}
                >
                  Xóa
          ...
          {contextMenu.kind === "database" && (
            <>
              <button
                role="menuitem"
                onClick={() => {
                  openSqlite(contextMenu.item.path);
                  setContextMenu(null);
                }}
              >
                Mở database
              ...
              <button
                role="menuitem"
                onClick={() => {
                  setSelectedSqlite(contextMenu.item.path);
              ...
              {currentUser?.role === "root" && !contextMenu.item.protected && (
                <button
                  role="menuitem"
                  className="danger"
                  onClick={() => {
                    deleteSqlite(contextMenu.item.path);
                    setContextMenu(null);
                  }}
                >
                  Xóa database
          ...
          {contextMenu.kind === "object" && (
            <>
              {["table", "view"].includes(contextMenu.item.type) && (
                <button
                  role="menuitem"
                  onClick={() => {
                    setSqliteWorkspace("data");
                    loadSqliteRows(selectedSqlite, contextMenu.item.name, 0, {
                      q: "",
                      sort: "",
                    });
                    setContextMenu(null);
              ...
              <button
                ...
                onClick={async () => {
                  await navigator.clipboard.writeText(
                    contextMenu.item.sql || contextMenu.item.name,
                  );
              ...
              {contextMenu.item.type === "table" &&
                currentUser?.role === "root" && (
                  <button
                    ...
                    className="danger"
                    onClick={() => {
                      setSelectedSqliteTable(contextMenu.item.name);
      ...
      <div className="toast-region" aria-live="polite" aria-label="Thông báo">
        {toasts.map((toast) => (
          <div
            key={toast.id}
          ...
          >
            ...
            {toast.duration > 0 && <span className="toast-progress" />}
          </div>
        ))}
      </div>
      {stepUpPrompt && (
        <div className="fixed inset-0 z-[100] bg-black/85 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="app-modal w-full max-w-md p-6 space-y-4 !border-rose-500/25">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="mt-1 text-xs text-slate-400">
                  Quyền xác nhận có hiệu lực 5 phút cho session hiện tại.
                </p>
              </div>
            </div>
            <input
            ...
            <input
              ...
              placeholder="Mã 2FA hoặc recovery code (nếu đã bật)"
              autoComplete="one-time-code"
              className="w-full bg-black border border-white/10 rounded px-3 py-2 text-sm"
            />
            {stepUpError && (
              <p className="text-xs text-red-400">{stepUpError}</p>
            )}
            <div className="flex justify-end gap-2">
              <button
              ...
              <button
                ...
                className="px-4 py-2 text-xs bg-red-600 text-white rounded disabled:opacity-40"
              >
                Xác nhận
              </button>
            </div>
